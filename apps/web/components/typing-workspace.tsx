...
export function TypingWorkspace() {
  ...
  return <main>
    <header><Brand/><nav>{!online&&<span className="offline" role="status">Offline</span>}<Link className="icon-link" href="/learn" title="Learn touch typing" data-tour="learn"><GraduationCap size={17}/></Link><Link className="icon-link" href="/leaderboard" title="Leaderboard"><Trophy size={17}/></Link>{user?.role==="user"&&<><Link className="icon-link" href="/history" title="Result history"><History size={17}/></Link><Link className="icon-link" href="/friends" title="Friends"><UserRoundCheck size={17}/></Link></>}<button title="Multiplayer" data-tour="multiplayer" disabled={!online} onClick={()=>user?setMulti(true):setAuthOpen(true)}><Users size={17}/></button><button title="Toggle theme" onClick={()=>setTheme(value=>value==="dark"?"light":"dark")}>{theme==="dark"?<Moon size={17}/>:<Sun size={17}/>}</button><div className="settings-wrap" data-tour="settings"><button title="Settings" aria-expanded={settingsOpen} onClick={()=>setSettingsOpen(value=>!value)}><Settings size={17}/></button>{settingsOpen&&<div className="settings-popover"><strong>Settings</strong><p>Review Typeflow features and typing controls.</p><button className="secondary" onClick={()=>{setSettingsOpen(false);window.dispatchEvent(new Event(restartTourEvent))}}><CircleHelp size={15}/> Start product guide</button></div>}</div>{user?<><span className="user-name">{user.displayName}</span><button title="Log out" onClick={logout}><LogOut size={17}/></button></>:<button className="account" disabled={!online} onClick={()=>setAuthOpen(true)}>Sign in</button>}</nav></header>
    ...
    <section className="race" data-tour="typing-area">
      <div className="test-status"><div className="status-metric"><b>{engine.wpm}</b><span>wpm</span></div><div className="timer-readout" aria-live="polite"><b>{engine.remaining}</b><span>{testKind==="time"?"seconds left":"seconds elapsed"}</span></div><div className="status-metric status-accuracy"><b>{engine.accuracy}%</b><span>accuracy</span></div><div className="test-progress"><i style={{width:`${testKind==="time"?Math.min(100,(duration-engine.remaining)/duration*100):engine.progress}%`}}/></div></div>
