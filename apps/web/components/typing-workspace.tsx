...
import { CircleHelp, GraduationCap, Moon, RotateCcw, Settings, Sun, Trophy, Users } from "lucide-react";
...
export function TypingWorkspace() {
  ...
  if(activeRace&&user)return <main><Multiplayer {...activeRace} onClose={()=>setActiveRace(null)}/></main>;
  return <main>
    <header><Brand/><nav>{!online&&<span className="offline" role="status">Offline</span>}<Link className="icon-link" href="/learn" title="Learn touch typing" data-tour="learn"><GraduationCap size={17}/></Link><Link className="icon-link" href="/leaderboard" title="Leaderboard"><Trophy size={17}/></Link><button title="Multiplayer" data-tour="multiplayer" disabled={!online} onClick={()=>user?setRaceSetupOpen(true):setAuthOpen(true)}><Users size={17}/></button><div ref={settingsRef} className="settings-wrap" data-tour="settings"><button title="Settings" aria-expanded={settingsOpen} onClick={()=>setSettingsOpen(value=>!value)}><Settings size={17}/></button>{settingsOpen&&<div className="settings-popover"><strong>Settings</strong><div className="settings-group"><span>Appearance</span><div className="settings-options"><button title="Use dark mode" aria-label="Use dark mode" data-active={theme==="dark"} onClick={()=>setTheme("dark")}><Moon size={16}/></button><button title="Use light mode" aria-label="Use light mode" data-active={theme==="light"} onClick={()=>setTheme("light")}><Sun size={16}/></button></div></div><div className="settings-group"><span>Help</span><button className="secondary" onClick={()=>{setSettingsOpen(false);window.dispatchEvent(new Event(restartTourEvent))}}><CircleHelp size={15}/> Start product guide</button></div></div>}</div>{user?<AccountMenu user={user} logout={logout}/>:<button className="account" disabled={!online} onClick={()=>setAuthOpen(true)}>Sign in</button>}</nav></header>
    <InviteNotifications enabled={user?.role==="user"} onAccept={(inviteCode,options)=>setActiveRace({...options,inviteCode})}/>
